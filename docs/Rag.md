...
- [x] 向量索引 (Qdrant 本地持久化 + VectorStoreIndex)
  ...
  - [x] Qdrant 向量資料庫
  - [x] 索引建立與載入
- [x] 查詢引擎 (RetrieverQueryEngine + Gemini 回答生成)
  - [x] GoogleGenAI LLM
  - [x] 回應輸出格式化
  - [x] 基本查詢引擎設定
    - [x] VectorIndexRetriever
    - [x] SimilarityPostprocessor
...
- **LLM 模型**：`GoogleGenAI(model="gemini-3.1-flash-lite")`。
- **API Key 載入**：透過 `.env` 與 `GEMINI_RAG_QUERY_ENGINE_API_KEY`。
- **檢索器**：使用 `VectorIndexRetriever(index=index, similarity_top_k=5)`，每次回傳前 5 筆相關節點。
- **回答生成器**：透過 `get_response_synthesizer(llm=llm)` 將檢索到的節點組合成回應。
- **後處理**：加入 `SimilarityPostprocessor(similarity_cutoff=0.7)`，過濾相似度過低的節點。
- **查詢管線**：以 `RetrieverQueryEngine` 串接 retriever、response synthesizer 與 node postprocessors，形成完整查詢流程。
