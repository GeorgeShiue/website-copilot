...
| 08/09 | 實作：M0（依賴升級）、M1（Agent CLI）、M2（多輪 + 串流）完成                                                          |
| 08/10 | 實作：M3（FastAPI SSE）、M4a（iframe + widget）完成；瀏覽器實測全過                                                   |
| 08/11 | 實作：M4b（Chrome Extension）、M5（收斂：測試/清理/CORS/文件）完成                                                     |

**結論**：以「最簡單的實證」打通價值鏈 —— **RAG 工具 → Agent → 可嵌入網站的聊天介面**。Phase 1 已提供 `webpage_retriever`（LangChain `StructuredTool`），本規劃決定以 **LangGraph `create_agent`** 包裝為 Agent（與 `2026_0721-RAG_Upgrade.md` §四 規劃一致），以 **FastAPI + SSE + 單頁前端** 提供聊天介面，並以 **iframe / script widget / Chrome Extension** 三種表面嵌入網站。分 M0~M5 六個里程碑漸進實作，先 CLI 驗證 Agent、再接介面。M0~M5 全部完成（2026/08/11）。
...
- [X] **M1**：`src/app/agent/`（agent.py），LLM 沿用 RAG config — ✅ 2026/08/09 完成
  ...
  - `src/app/agent/agent.py`（Agent wrapper、create_agent、ask_agent、來源擷取）
  ...
  - **M1 後續變更**（2026/08/09）：
    ...
    - `agent_cli.py` 合併進 `src/cli.py`：新增 `agent-cli` subcommand（執行：`uv run python src/cli.py agent-cli --run.query "問題"`）；`agent.py`（Agent 層）不變，agent_cli.py 已刪除
    ...
    - session log 收斂：移除 3 個空洞 session（`Wrapping as StructuredTool` / `RAG Retriever Tool Created` / `Agent Created`，由 `log_run_time` 的 `Completed in ...` 收尾）；`Building Agent` 補 `INFO` 內容（llm / tools / system_prompt 長度）；`agent.py` 起點補 `log_config`
...
- [x] **M2**：MemorySaver + astream 串流 — ✅ 2026/08/09 完成
  - `agent.py`：`Agent` 新增 `checkpointer`（`InMemorySaver`，`MemorySaver` 的新名稱）；`ask_agent` 支援 `thread_id` 多輪記憶；新增 `astream_agent()`（`stream_mode="messages"` 逐 token，僅 model 節點）
...
- [x] **M3**：FastAPI SSE endpoint — ✅ 2026/08/10 完成
  ...
  - 資源生命週期：agent 於 lifespan 啟動建一次、關閉釋放（create_agent 會重建 vector store 隔離副本，不可 per-request）；對話隔離靠 thread_id（InMemorySaver）
...
- [x] **M4b**（加分）：Chrome Extension 本機開發模式 — ✅ 2026/08/11 完成
  - **M4b-1 widget.js 重構**：IIFE → `WebsiteCopilotWidget.mount(options)` factory（endpoint / threadId / streamChat 成為 mount 內狀態）；新增 transport 抽象 `streamChat(payload)`（預設 `defaultStreamChat(endpoint)` 直接 fetch，content.js 可傳 chrome.runtime 代理版）；自動掛載相容（`document.currentScript` 存在時自動 mount，content script 環境 currentScript 為 null 不自動掛載）。驗證：node --check + 瀏覽器實測網頁版（demo.html widget/iframe 串流、markdown、來源連結全正常）
  - **M4b-2 extension 三件套**（`extension/`）：
    ...
    - `background.js`：`chrome.runtime.onConnect`（wc-chat port）→ fetch 後端 SSE → 逐塊 `port.postMessage({type:'chunk'})` → 完成 `done`；`AbortController` 於 content 斷線時中止（fetch 進行中 SW 不休眠，chunk 兼 keepalive）
    - `content.js`：`proxyStreamChat(payload)` 建立 port + `ReadableStream` 轉接（依 `\n\n` 切 SSE 區塊）→ `WebsiteCopilotWidget.mount({endpoint, streamChat})`；`__wcMounted` 防重複掛載
    - `widget.js`：實體複本（**Chrome 不載入 symlink 的 content script**，踩坑），頂部加同步註記 `cp src/app/server/static/widget.js extension/widget.js`
  - **踩坑**：Playwright 載入 extension 必須 `launch_persistent_context` + headed（無 DISPLAY 需 xvfb-run）；headless / 普通 context 不載入 extension；content.js pump 的 `\n\n` 切割 bug（slice 不含結尾導致 widget 端 slice 吃掉首字元、串流隨機中斷）→ 修正 `slice(0, idx+2)`
  - **M4b-3 自動化**（`scripts/m4b_extension_test.py`）：一條指令端到端——無 DISPLAY 自動 `xvfb-run` 重跑、自動啟動後端（或 `--base-url` 用既有）、自動起測試頁（port 18234）、Playwright 載入 extension 驗證（注入 → 開對話框 → 第一問串流穩定 → markdown strong/list/link → 多輪追問含來源連結）、自動清理。執行：`uv run python scripts/m4b_extension_test.py`；實測 M4B OK（340 + 590 字元、markdown 全 True、多輪含引用）
  - 手動測試（2026/08/11）：chrome://extensions load unpacked → 在 Google Sites 浮出 💬 → 對話、markdown、來源引用正常；widget 輸入框補 `name="wc-query"`（消除 Lighthouse form field warning）
  - 驗收指標（doc §8）達成：載入 extension 後在任意網站浮出 widget 並可對話 ✅
- [x] **M5**：測試、文件、README 收斂 — ✅ 2026/08/11 完成
  - **M5-1 測試補齊**：新增 `src/test/test_agent.py`（12 個純函式測試：thread_config / extract_sources_from_messages / _message_content_to_text / save_conversation_results）；`test_server.py` +2 CORS 測試（header / preflight）；`test_module.py` 標記 `slow`（pyproject 註冊 marker）；快速路徑 `pytest -m "not slow"` 24 passed、4 deselected
  - **M5-2 程式碼清理**：多輪落盤依 thread_id 分檔（`save_conversation_results` 加 `thread_id` 參數 → `results_<thread_id>.json`；`RunManager.save_results_as_json` 合併 `file_path` 參數）；死碼掃描（ruff F401/F841/F811）全過；server 與 CLI 共用 `astream_text` 核心（不強行合併 `_event_stream` / `astream_agent_result`，避免 queue 橋接 race）
  - **M5-3 CORS 限縮**：`create_app` / `run_server` 加 `allowed_origins`（None 全開放）；`ServerRunConfig.allowed_origins`；+2 測試（清單內允許 / 清單外拒絕）
  - **M5-4 README 全面更新**：功能補 Phase 2/3（Agent / 聊天介面 / 嵌入）、檔案結構校正（engines/ agent/ server/ extension/ scripts/ chats/）、使用方式補 agent-cli / server-cli / 嵌入三方式 / 驗證腳本、環境變數補 Agent 用途、新增聊天記錄章節
  - **M5-5 文件收斂**：`docs/project.md` Phase 2/3 改完成狀態 + 文件連結；新增 `docs/code/phase2_3_mvp/phase2_3_mvp.md`（參照 phase1.md 寫法）；`docs/code/runs/cli.md` 補 agent-cli / server-cli；repo memory 兩份更新（src_layout / agent_phase2_3）
