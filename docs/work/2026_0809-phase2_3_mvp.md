...
- [x] **M2**：MemorySaver + astream 串流 — ✅ 2026/08/09 完成
  ...
  - **M2 後續調整**（2026/08/09）：`MemorySaver` → `InMemorySaver`（langgraph-checkpoint 新名稱，兩者共存，agent.py 已全面更新）；cli.py 串流 helper 由 `_run_agent_stream` 改名為 `stream_agent`（公開命名，供 M3 server 重用）
- [ ] **M3**：FastAPI SSE endpoint
- [ ] **M4a**：widget.js + chat.html + demo.html + CORS
