...
涵蓋：
- create_tool 回傳 tools 列表
- tools 結構正確（名稱、數量、型別）
- registry 由外部傳入，create_tool 不修改它
...
def test_create_tool_returns_list(
    ...
    result = create_tool(registry=mock_registry)
...
@patch("app.tools.tool.create_webpage_retriever_tool")
@patch("app.tools.tool.create_site_discovery_tool")
def test_create_tool_tools_correct_names(
    mock_create_discovery,
    mock_create_retriever,
):
    ...
    tools = create_tool(registry=mock_registry)

    assert len(tools) == 2
    assert tools[0].name == "list_knowledge_bases"
    assert tools[1].name == "webpage_retriever"


@patch("app.tools.tool.create_webpage_retriever_tool")
@patch("app.tools.tool.create_site_discovery_tool")
def test_create_tool_passes_registry_to_tool_creators(
    mock_create_discovery,
    mock_create_retriever,
):
    """create_tool 將 registry 傳入工具建立函數。"""
    from app.tools.tool import create_tool
    ...
    create_tool(registry=mock_registry)
...
@patch("app.tools.tool.create_webpage_retriever_tool")
@patch("app.tools.tool.create_site_discovery_tool")
def test_create_tool_does_not_modify_registry(
    mock_create_discovery,
    mock_create_retriever,
):
    """create_tool 不觸發任何 RAG 建構操作。"""
    from app.tools.tool import create_tool
