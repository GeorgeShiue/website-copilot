...
LLM_CONFIG = LLMConfig(
    ...
    api_token=os.getenv("OPENAI_CRAWL4AI_TEST_API_KEY"),
)
...
async def demo_fit_markdown():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        with open(f"{__cur_dir__}/tmp/raw_markdown.md", "w") as f:
            f.write(result.markdown.raw_markdown)
        with open(f"{__cur_dir__}/tmp/fit_markdown.md", "w") as f:
            f.write(result.markdown.fit_markdown)


async def demo_llm_structured_extraction_no_schema():
    # Create a simple LLM extraction strategy (no schema required)
    print("\n=== 4. LLM-Based Structured Extraction (No Schema) ===")
...
async def demo_raw_html_and_file():
    ...
    # Save to file
    file_path = f"{__cur_dir__}/tmp/sample.html"
